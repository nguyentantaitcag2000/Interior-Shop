...
const changeAmount = (event:Event) => {
    ...
    if(button.className.includes('btn-minuse'))
        ammount.value = ammount.value > 1 ? ammount.value -1 : ammount.value;
    else
        ammount.value++;
}
onMounted(()=>{
...
<template>
    <div class="container p-5 mt-5 bg-light" style="border-radius: 25px;">
        <div class="row w-100">
            ...
            <div class="col-lg-8 w3-animate-bottom">
                <div class="pl-3 ml-3">
                    ...
                    <form method="POST" :action="'/Home/Checkout/?>' + myProduct?.ID_Product">
                        <span><b>Màu sắc:</b> {{ myProduct?.detail_product_color.map(item => item.color.Name_Color).join(', ') }}</span><br>
                        <span><b>Kích thước:</b> {{ myProduct?.Size }}</span><br>
                        <span><b>Chất liệu:</b> {{ myProduct?.detail_product_material.map(item=> item.material.Name_Material).join(', ') }}</span><br>
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-auto">
                                <div class="input-group" style="width:50%">
                                    ...
                                    <input v-model="ammount" type="text" class="form-control no-padding add-color text-center height-25" maxlength="3">
                                    <span class="input-group-btn">
                        ...
                        <div class="row m-3">
                            <button class="btn btn-primary" onclick="AddCart(event)"><i class="fas fa-shopping-cart mr-2" ></i>Đặt vào giỏ hàng</button>
                        
                            <input class="btn btn-success ml-3" value="Mua" type="submit" >
