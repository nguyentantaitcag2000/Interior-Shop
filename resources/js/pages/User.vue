...
function loadUserType()
{
    axios.post('/api/user-type').then(res=>{
        ...
        let indexAdmin = user_types.value?.findIndex(u => u.Name_UT == 'admin');
        if(indexAdmin !== -1)
        {
            user_types.value?.splice(indexAdmin!, 1);
        }
            ...
            <DataTable v-model:filters="filters" :value="users" dataKey="ID_User" tableStyle="min-width: 50rem" showGridlines stripedRows
                ...
                <template #header>
                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                        <span class="text-xl text-900 font-bold">Danh sách người dùng:</span>
                ...
                <Column   header="ID khách hàng">
                    <template #body="slotProps">
                        <span>{{ slotProps.data.ID_User }}</span>
                    </template>
                </Column>
                
                <Column header="Tên khách hàng" sortable>
                    <template #body="slotProps">
